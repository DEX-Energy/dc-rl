...
> This codebase is for the paper: [SustainDC: Benchmarking for Sustainable Data Center Control](https://openreview.net/pdf?id=UYgE9IfQIV), which is under review for the NeurIPS 2024 Datasets and Benchmarks Track.
...
Workload trace files should be in CSV format, with two columns: a timestamp or index (must be unnamed), and the corresponding DC Utilization (`cpu_load`). The CPU load must be expressed as a fraction of the DC utilization (between 0 and 1).

**Dataset Length Requirements** (updated Nov 2025):
